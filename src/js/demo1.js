App = {
  ...
  initContract: function() {
    $.getJSON('Crowdsale1.json', function(data) {
      // Get the necessary contract artifact file and instantiate it with truffle-contract.
      var Crowdsale1Artifact = data;
      App.contracts.Crowdsale1 = TruffleContract(Crowdsale1Artifact);
      // Set the provider for our contract.
      App.contracts.Crowdsale1.setProvider(App.web3Provider);

      App.contracts.Crowdsale1.deployed().then(function(instance) {
        var Crowdsale1 = instance;
        ...
        instance.FundTransfer({}, '', function(error, result){
          if (!error)
            if (result.args.isContribution) {
                ...
                console.log( "\n The current funding at " +( 100 *  Crowdsale1.camountRaised.call() / Crowdsale1.cfundingGoal.call()) + "% of its goals.");

                App.syncAllInfo();
                App.getTokenBalances();
            } else {
                console.log("Funds transferred from Crowdsale1 account: " + result.args.amount * Math.pow(10, -18) + " ether to " + result.args.backer  )
            }
        });
      });
  ...
  handleTransfer: function(event) {
   ...
   //   console.log('Account unlocked ' + result);
      App.contracts.Crowdsale1.deployed().then(function(instance) {
         ...
         console.log('Crowdsale1Closed: ' + result);
         if (!result) {
           // App.contracts.Crowdsale1.deployed().then(function(instance) {
           ...
           // }).then(function(result) {
              ...
              App.contracts.Crowdsale1.deployed().then(function(instance) {
                 return instance.sendTransaction({from: App.myaccount, to: instance.address, value: web3.toWei(amount, 'ether'), gas: 3000000});
              }).then(function(result) {
      ...
      // }).catch(function(err) {
      //    alert("Crowdsale1Closed: " + err.message);
      // });
  ...
  handleTokenTransfer: function(event) {
    ...
    //Crowdsale1.deployed().then(function(instance) { return instance.ttransfer('0xf17f52151ebef6c7334fad080c5704d77216b732', 1); }).then(function(value){return value; });
    App.contracts.Crowdsale1.deployed().then(function(instance) {
